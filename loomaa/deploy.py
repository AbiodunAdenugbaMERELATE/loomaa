...
def deploy_monolithic_tmdl(model_name: str, tmdl_path: str, delete_first: bool = False):
    ...
    print(f"[loomaa] 🚀 Deploying monolithic TMDL for '{model_name}' -> {tmdl_path}")
    ...
    if resp.status_code in (200, 201):
        print("[loomaa] ✅ Definition updated successfully")
    ...
    if resp.status_code == 202:
        ...
        print("[loomaa] ⏳ Polling update operation...")
        op_result = wait_for_operation_completion(op_url, access_token, retry_after)
        status = op_result.get('status')
        if status == 'Succeeded':
            print("[loomaa] ✅ Monolithic update completed successfully")
        else:
            print(f"[loomaa] ⚠️ Update operation ended with status {status}")
        return {"status": status, "id": model_id, "operationResult": op_result}
...
def deploy_complete_semantic_model(model_name, semantic_model_path):
    ...
    print(f"[loomaa] 🚀 Deploying complete semantic model '{model_name}' using Fabric REST API...")
    ...
    try:
        ...
        if existing_model_id:
            ...
            print(f"[loomaa] ✅ Existing model deleted successfully")
        ...
        if response.status_code in [200, 201]:
            ...
            try:
                ...
                print(f"[loomaa] ✅ Semantic model '{model_name}' deployed successfully!")
            ...
            if operation_url:
                print(f"[loomaa] ⏳ Deployment started (asynchronous). Checking status...")
                ...
                print(f"[loomaa] ✅ Semantic model '{model_name}' deployment accepted!")
        ...
        print(f"[loomaa] ❌ Deployment failed: {e}")
        raise
...
def deploy_model_fabric_api(model_name, tmdl_path):
    ...
    print(f"[loomaa] 🚀 Deploying model '{model_name}' using Fabric REST API...")
...
def wait_for_operation_completion(operation_url, access_token, initial_delay=20):
    ...
    # Timeout reached
    print(f"[loomaa] ⚠️  Operation timeout reached. Deployment may still be processing in background.")
...
def deploy_semantic_model(model_name, semantic_model_path):
    ...
    try:
        ...
        print(f"[loomaa] ✅ Model '{model_name}' deployed successfully!")
...
def ensure_powershell7():
    ...
    try:
        ...
        if result.returncode == 0:
            print("[loomaa] ✅ PowerShell 7 found")
            return "pwsh"
    ...
    for path in common_paths:
        if os.path.exists(path):
            print(f"[loomaa] ✅ PowerShell 7 found at: {path}")
            return path
    
    # PowerShell 7 not found - provide helpful instructions
    print("\n[loomaa] ❌ PowerShell 7 required for deployment but not found.")
    print("\n📖 Installation Instructions:")
    print("   Local Development:")
    print("   • Download from: https://github.com/PowerShell/PowerShell/releases")
    print("   • Or use winget: winget install Microsoft.PowerShell")
    print("   • Or use Chocolatey: choco install powershell-core")
    print("\n   CI/CD Environments:")
    print("   • Azure DevOps: PowerShell 7 is pre-installed (use pwsh: true)")
    print("   • GitHub Actions: PowerShell 7 is pre-installed")
    print("\n💡 After installation, restart your terminal and try again.")
    print("\n📚 Documentation: https://loomaa.dev/deployment")
...
def create_semantic_model_structure(model_name, tmdl_path):
    ...
    print(f"[loomaa] ✅ Created .SemanticModel structure")
    return pbip_folder
...
def deploy_with_powershell(pwsh_path, model_name, pbip_folder, creds):
    ...
    Write-Host "[loomaa] ✅ Modules ready"
    ...
    try {{
        ...
        Write-Host "[loomaa] ✅ Authentication successful"
    }} catch {{
    ...
    try {{
        ...
        if ($result) {{
            Write-Host "[loomaa] ✅ Deployment successful!"
